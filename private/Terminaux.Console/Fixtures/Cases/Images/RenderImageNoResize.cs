...
{
    internal class RenderImageNoResize : IFixture
    {
        ...
        public void RunFixture()
        {
            ...
            {
                if (!pictureName.StartsWith("Terminaux.Console.Assets.pictures."))
                    continue;
                var stream = asm.GetManifestResourceStream(pictureName);
                if (stream is null)
                    return;
                string rendered = ImageProcessor.RenderImage(stream, 40, 20, 4, 2, resize: false);
