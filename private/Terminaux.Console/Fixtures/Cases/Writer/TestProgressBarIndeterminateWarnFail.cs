...
{
    internal class TestProgressBarIndeterminateWarnFail : IFixture
    {
        ...
        public void RunFixture()
        {
            ...
            var progressBar = new ProgressBar(
                "This is the test progress bar that contains a scrolling marquee. It will warn at 50%, then fail at 75%.", 0, 100)
            {
            ...
            try
            {
                ...
                // Finally, increment the progress bar until it fails at 75%, but visually warn at 50%
                for (int progress = 0; progress < 100; progress++)
                {
                    progressBar.Position = progress;
                    if (progress == 75)
                    {
                        progressBar.State = ProgressState.Failed;
                        Thread.Sleep(5000);
                        break;
                    }
                    else if (progress == 50)
